...
class DenseboxHead(ModuleBase):
    ...
    def _make_conv3x3(self):
        ...
        # head_width_in = int(head_width * dist_cmprate['backbone_conv5'])

        head_width_cls_in = int(head_width * dist_cmprate['tranf_c_x'])
        head_width_reg_in = int(head_width * dist_cmprate['tranf_r_x'])
        head_width_cls_out = int(head_width * dist_cmprate['head_cls_conv3x3_1'])
        head_width_reg_out = int(head_width * dist_cmprate['head_bbox_conv3x3_1'])

        cls_conv3x3 = conv_bn_relu(head_width_cls_in,
                                   head_width_cls_out,
                                   stride=1,
                                   kszie=3,
                                   pad=0,
                                   has_bn=head_conv_bn[0])

        bbox_conv3x3 = conv_bn_relu(head_width_reg_in,
                                    head_width_reg_out,
                                    stride=1,
                                    kszie=3,
                                    pad=0,
                                    has_bn=head_conv_bn[0])
        setattr(self, 'cls_p5_conv%d' % (1), cls_conv3x3)
        setattr(self, 'bbox_p5_conv%d' % (1), bbox_conv3x3)
        self.cls_conv3x3_list.append(cls_conv3x3)
        self.bbox_conv3x3_list.append(bbox_conv3x3)

        head_width_cls_in = head_width_cls_out
        head_width_reg_in = head_width_reg_out
        head_width_cls_out = int(head_width * dist_cmprate['head_cls_conv3x3_2'])
        head_width_reg_out = int(head_width * dist_cmprate['head_bbox_conv3x3_2'])
        ...
        bbox_conv3x3 = conv_bn_relu(head_width_reg_in,
                                    ...
                                    pad=0,
                                    has_bn=head_conv_bn[1])
        setattr(self, 'cls_p5_conv%d' % (2), cls_conv3x3)
        setattr(self, 'bbox_p5_conv%d' % (2), bbox_conv3x3)
        self.cls_conv3x3_list.append(cls_conv3x3)
        self.bbox_conv3x3_list.append(bbox_conv3x3)

        head_width_cls_in = head_width_cls_out
        head_width_reg_in = head_width_reg_out
        head_width_cls_out = int(head_width * dist_cmprate['head_cls_conv3x3_3'])
        head_width_reg_out = int(head_width * dist_cmprate['head_bbox_conv3x3_3'])
        ...
        bbox_conv3x3 = conv_bn_relu(head_width_reg_in,
                                    ...
                                    pad=0,
                                    has_bn=head_conv_bn[2])
        setattr(self, 'cls_p5_conv%d' % (3), cls_conv3x3)
        setattr(self, 'bbox_p5_conv%d' % (3), bbox_conv3x3)
        self.cls_conv3x3_list.append(cls_conv3x3)
        self.bbox_conv3x3_list.append(bbox_conv3x3)
    ...
    def _make_conv_output(self):
        ...
        out_layer_cls_in = int(head_width * dist_cmprate['head_cls_conv3x3_3'])
        out_layer_cls_out = 1
        out_layer_reg_in = int(head_width * dist_cmprate['head_bbox_conv3x3_3'])
        out_layer_reg_out = 1
        self.cls_score_p5 = conv_bn_relu(out_layer_cls_in,
                                         1,
                                         stride=1,
                                         kszie=1,
                                         pad=0,
                                         has_relu=False)
        self.ctr_score_p5 = conv_bn_relu(out_layer_cls_in,
                                         1,
                                         stride=1,
                                         kszie=1,
                                         pad=0,
                                         has_relu=False)
        self.bbox_offsets_p5 = conv_bn_relu(out_layer_reg_in,
                                            4,
                                            stride=1,
                                            kszie=1,
                                            pad=0,
